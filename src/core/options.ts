...
export const MAP_OPTIONS = {
   ...
   *
   * ★ 2026-09-21 备注：**"标签字发虚"这件事，本条正是候选改法之一（那条成本最高、效果最锐）。**
   *   完整根因、四条候选改法与各自代价，记在 `map-2d/scripts/gen-glyphs.cs` 文件尾部的
   *   「附着：标签清晰度现状与候选改法」；需求方当时决定**本轮放弃、后续再改**。
   *   动这一行之前，务必先读那段备注里的"代价 ②"（混排被拆成两块）与导出影响。
   */
   ...
   * 生成范围以 `scripts/.regen-radius8.ps1`（当前出货版本）为准：GB2312 一级汉字 + 应用源码用字
   * = 3758 个，铺开 **84 段，约 3.6 MB**（**不是**早先"只造非表意字符、577 KB"那版，那句已过期）。
